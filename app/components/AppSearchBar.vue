...
function goToVideo(result: any) {
  const playlistId = result.item?.playlist_id
  if (playlistId) {
    router.push(`/playlist/${playlistId}?videoId=${result.video?.id}`)
  }
  clear()
}
</script>
  ...
  <div class="relative w-full">
    ...
    >
      ...
      <button
        ...
        :key="result.item?.id"
        class="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 text-left"
        role="option"
        @click="goToVideo(result)"
      >
        <img
        ...
        <div class="min-w-0">
          ...
          <p class="text-xs text-gray-500 truncate">{{ result.video?.channel_title }}</p>
        </div>
      </button>
    </div>
  </div>
